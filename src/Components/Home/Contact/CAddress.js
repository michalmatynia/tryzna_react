import React from 'react';

const ConDetails = () => {
    return (
        <div>
            <p><strong>Contact Us</strong></p>
            <p>If you have any queries regarding current and future projects or would like to ask for pricing quotation in relation to your prospected visualization, please contact us with the details below.</p>

            <p>The <strong>Office</strong></p>
            <p><strong>Address:</strong> 71 Fioletowa, Szczecin, Poland</p>
                <p><strong>Phone:</strong> +48 798 985 231</p>
                <p><strong>Email:</strong> <a href="mailto:info@tryzna.pl">info@tryzna.pl</a></p>
                <p>Opening <strong>Hours</strong></p> <p>Monday - Friday 9am - 5pm</p> <p> Saturday - Closed</p> <p> Sunday - Closed</p>

        </div>
    );
};

export default ConDetails;

